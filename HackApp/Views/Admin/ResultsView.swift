...
struct ResultsView: View {
    ...
    @State private var scores: [String: Double] = [:] // Calificaciones generales de los equipos
    @State private var calificacionesPorCriterio: [String: [String: Double]] = [:] // Calificaciones por criterio
    @State private var topTeams: [(team: String, score: Double)] = [] // Mejor puntuados por puntajes generales
    @State private var topTeamsPorCriterio: [(team: String, score: Double)] = [] // Mejor puntuados por criterio
    ...
    var body: some View {
        VStack {
            ...
            if isLoading {
                ...
                // Picker para seleccionar el criterio
                Picker("Seleccionar Criterio", selection: $selectedCriterio) {
                    Text("Puntaje General").tag(nil as String?)
                ...
                .onChange(of: selectedCriterio) { _ in
                    updateTopTeamsPorCriterio() // Actualizamos el ranking cuando cambiamos el criterio
                }

                // Mostrar gráfico de barras dependiendo de la selección del picker
                if let criterio = selectedCriterio, let calificaciones = calificacionesPorCriterio[criterio] {
                    // Mostrar gráfico para el criterio seleccionado
                    ...
                    // Mostrar gráfico para el puntaje general
            ...
            Text(selectedCriterio != nil ? "Mejores Equipos por \(selectedCriterio!)" : "Mejores Equipos")
                .font(.title2)
                .fontWeight(.bold)
                .padding(.top)
    ...
    func fetchScores() {
        ...
        viewModel.getScores(for: hack.clave) { result in
            ...
            case .failure(let error):
                print("Error al obtener los puntajes generales: \(error)")
            }
        }
        ...
        viewModel.getCalificacionesPorCriterio(for: hack.clave) { result in
            ...
            case .success(let calificacionesPorCriterio):
                ...
                self.updateTopTeamsPorCriterio() // Actualizamos los equipos por criterio seleccionado
            case .failure(let error):
    ...
    private func updateTopTeamsPorCriterio() {
        // Si hay un criterio seleccionado, lo actualizamos
